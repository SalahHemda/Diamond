...
@Client.on_message(command(["تشغيل","شغل","play","/play","صلاح", f"play@{BOT_USERNAME}"]) & other_filters)
        ...
        try:
            ...
            if m.chat.username:
                try:
                    ...
                    await m.reply_text(f"❌ **فشل انضمام المساعد**\n\n**السبب**: `{e}`")
                    return
            else:
                try:
                    ...
                    return await m.reply_text(
                        f"❌ **فشل انضمام المساعد**\n\n**السبب**: `{e}`"
                    )
        if replied:
            if replied.audio or replied.voice:
                suhu = await replied.reply("**ثواني بس كدا هبحث عنها.**")
                ...
                if chat_id in QUEUE:
                    ...
                    try:
                        ...
                        await suhu.delete()
                        await m.reply_text(f"🚫 حدث خطأ َتأكد من فتح مكالمه اولا:\n\n» {e}")
            else:
                if len(m.command) < 2:
                    await m.reply(
                        "» قم بالرد على ملف صوتي او اكتب اسم اغنيه للبحث عنهاّ**"
                    )
                else:
                    ...
                    if search == 0:
                        ...
                        if veez == 0:
                            ...
                            if chat_id in QUEUE:
                                ...
                                await m.reply_photo(
                                    photo=thumbnail,
                                    caption=f"💡 ***-› تم اضافتها للانتظار** `{pos}`\n\n🏷 **-› الاسم:** [{songname}]({url})| موسيقى`\n**⏱ المدهّ:** `{duration}`\n🎧 **-› طلب الكاين دا:** {requester}",
                                    reply_markup=keyboard,
                                )
                            else:
                                try:
                                    await suhu.edit("🔄 ** يتم التشغيلَ...**")
                                    await call_py.join_group_call(
                                        chat_id,
                                        AudioPiped(
                                            ytlink,
                                    ...
                                    await suhu.delete()
                                    await m.reply_text(f"🚫 حدث خطأ َتأكد من فتح مكالمه اولا: `{ep}`")

        else:
            if len(m.command) < 2:
                await m.reply(
                    "» الرد على ** ملف صوتي  ** أو ** اكتب اسم اغنيه للبحث عنهاّ.**"
                )
            else:
                ...
                if search == 0:
                    ...
                    if veez == 0:
                        ...
                        if chat_id in QUEUE:
                            ...
                            try:
                                await suhu.edit("🔄 **جاري الانضمام الى المكالمه...**")
                                await call_py.join_group_call(
                                    chat_id,
                                    AudioPiped(
                                        ytlink,
                                ...
                                await m.reply_photo(
                                    photo=thumbnail,
                                    caption=f"🏷 **-› الاسم:** [{songname}]({url})\n**⏱ المده:** `{duration}`\n💡 ** الحالة:** `يشغل`\n🎧 **-› طلب الكائن داٛ:** {requester}\n📹 ** نوع البثّ:** `موسيقى`",
                                    reply_markup=keyboard,
                                )
                            except Exception as ep:
                                await suhu.delete()
                                await m.reply_text(f"حدث خطأ غير متوقع تأكد من وجود مكالمه بالفعل{ep}`")